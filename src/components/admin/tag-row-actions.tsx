...
export function TagRowActions({ id, name, isActive, canDelete }: TagRowActionsProps) {
  ...
  async function toggleActive() {
    setPending("status");
    setError(undefined);
    const result = await setTagActiveAction(id, !isActive);
  ...
  return (
    <>
      <Button type="button" variant="outline" size="sm" className="h-10 rounded-md" disabled={Boolean(pending)} onClick={toggleActive}>
        {pending === "status" ? (
      ...
      {confirming && (
        ...
        >
          <section
            role="alertdialog"
            aria-modal="true"
            aria-labelledby={`delete-tag-title-${id}`}
            aria-describedby={`delete-tag-description-${id}`}
            className="w-full max-w-md rounded-md border bg-background p-5 shadow-xl"
          >
            <h2 id={`delete-tag-title-${id}`} className="text-lg font-semibold">Delete tag</h2>
            <p id={`delete-tag-description-${id}`} className="mt-2 text-sm text-muted-foreground">
              Permanently delete &quot;{name}&quot;? Tags in use cannot be deleted.
            </p>
