...
export function PasswordResetForm({ token }: { token: string }) {
  ...
  useEffect(() => {
    if (!complete) return;
    const redirectTimer = window.setTimeout(() => router.replace("/login?reset=success"), 1200);
    return () => window.clearTimeout(redirectTimer);
  }, [complete, router]);
  return <form action={action} className="space-y-5" noValidate><input type="hidden" name="token" value={token} />{state.status !== "idle" ? <p role={state.status === "error" ? "alert" : "status"} className={state.status === "error" ? "flex gap-2 rounded-[var(--public-radius)] border border-red-200 bg-red-50 px-3 py-3 text-sm text-red-800" : "flex gap-2 rounded-[var(--public-radius)] border border-green-200 bg-green-50 px-3 py-3 text-sm text-green-800"}>{state.status === "error" ? <CircleAlert className="size-5 shrink-0" aria-hidden="true" /> : <CircleCheck className="size-5 shrink-0" aria-hidden="true" />} {state.message}{complete ? " Redirecting to sign in…" : null}</p> : null}<PasswordInput id="password" name="password" label="New password" visible={visible} setVisible={setVisible} disabled={complete} /><PasswordInput id="confirm-password" name="confirmPassword" label="Confirm new password" visible={confirmVisible} setVisible={setConfirmVisible} disabled={complete} /><p className="rounded-[var(--public-radius)] border border-[#eadfcf] bg-[#fffaf1] px-4 py-3 text-sm leading-6 text-public-muted-text">Use at least 12 characters with uppercase, lowercase, and a number.</p>{complete ? <Link href="/login?reset=success" className="inline-flex h-[3.25rem] w-full items-center justify-center rounded-[var(--public-radius)] bg-public-primary text-base font-bold text-white hover:bg-public-primary-hover">Sign in now</Link> : <Button type="submit" disabled={pending} className="h-[3.25rem] w-full rounded-[var(--public-radius)] bg-public-primary text-base font-bold text-white hover:bg-public-primary-hover">{pending ? "Resetting password..." : "Reset password"}</Button>}</form>;
}
